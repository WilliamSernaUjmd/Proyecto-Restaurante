 ...
 * @author JCerna
 */
public class ConsultasTikets extends javax.swing.JInternalFrame {

    /**
     * Creates new form ConsultasTiket
     */
    public ConsultasTikets() {
        initComponents();
        cargartodasTikets();
    ...
    void cargartodasTikets() {
        ...
        try {
            ...
            Logger.getLogger(ConsultasTikets.class.getName()).log(Level.SEVERE, null, ex);
        }
...
// TODO add your handling code here:
    ...
    String consulta = "";
    if (rdbnnumero.isSelected() == true) {
        consulta = "SELECT * FROM tiket WHERE num_tik='" + num + "'";
    }
    if (rdbbnfecha.isSelected() == true) {
        Date fecha = jDateChooser1.getDate();
        SimpleDateFormat formatofecha = new SimpleDateFormat("dd/MM/YYYY");
        String fec = "" + formatofecha.format(fecha);
        consulta = "SELECT * FROM restaurante.tiket WHERE fecha='" + fec + "'";
    }
    if (rdbntodos.isSelected() == true) {
        consulta = "SELECT * FROM restaurante.tiket ";
    }
    DefaultTableModel tabla = new DefaultTableModel();
    String[] titulos = {"NUMERO", "COD. CLIENTE", "TOTAL A PAGAR", "FECHA"};
    tabla.setColumnIdentifiers(titulos);
    ...
    try {
        ...
        ResultSet rs = st.executeQuery(consulta);
        while (rs.next()) {
            Datos[0] = rs.getString("num_tiket");
            Datos[1] = rs.getString("cod_cli");
            Datos[2] = rs.getString("pre_tot");
            Datos[3] = rs.getString("fecha");
...
// TODO add your handling code here:
    ...
    if (filasele == -1) {
        ...
        DetalleTikets detalle = new DetalleTikets();
        ...
        DetalleTikets.txtfac.setText(numbol);
        DetalleTikets.txtcod.setText(cod);

        DetalleTikets.txttot.setText(total);
        DetalleTikets.txtfecha.setText(fecha);
        DefaultTableModel model = (DefaultTableModel) DetalleTikets.tbdetalle.getModel();
        String ver = "SELECT * FROM restaurante.detalletiket WHERE num_tik='" + numbol + "'";
        String[] datos = new String[5];
        try {
            Statement st = cn.createStatement();
            ResultSet rs = st.executeQuery(ver);
            while (rs.next()) {
                datos[0] = rs.getString("cod_pla");
                datos[1] = rs.getString("des_pla");
                datos[2] = rs.getString("cant_pla");
                datos[3] = rs.getString("precio");
                datos[4] = rs.getString("pre_venta");
                model.addRow(datos);

            }
            DetalleTikets.tbdetalle.setModel(model);
        } catch (SQLException ex) {
            Logger.getLogger(ConsultasTikets.class.getName()).log(Level.SEVERE, null, ex);
        }
...
// TODO add your handling code here:
    ...
    if (fila >= 0) {
        ...
        try {
            PreparedStatement pst = cn.prepareStatement("DELETE FROM restaurante.tiket WHERE num_tik='" + cod + "'");
            pst.executeUpdate();
            cargartodasTikets();
        } catch (SQLException ex) {
            Logger.getLogger(ConsultasTikets.class.getName()).log(Level.SEVERE, null, ex);
